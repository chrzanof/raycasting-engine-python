...
class RaycastingEngine:
    ...
    def render(self, canvas):
        canvas.create_rectangle(0, SCREEN_HEIGHT/2, SCREEN_WIDTH, SCREEN_HEIGHT, fill=rgb_to_hex(FLOOR_COLOR_RGB), width=0)
    ...
    def cast_rays(self, canvas):
        ...
        px_rotated, py_rotated = return_rotated_actor_position(self.player.x, self.player.y, -90, len(self.level.level_map), len(self.level.level_map[0]))
        for i in range(0, number_of_rays):
            ...
            color_scale_dist = 1 - min(wall_dist / 16, 1)
            ...
            canvas.create_rectangle(screen_position_x, LEVEL_SCREEN_MARGIN_TOP + SCREEN_HEIGHT/2 - 0.5 * line_height,
                                    next_screen_position_x, LEVEL_SCREEN_MARGIN_TOP + SCREEN_HEIGHT/2 + 0.5 * line_height,
